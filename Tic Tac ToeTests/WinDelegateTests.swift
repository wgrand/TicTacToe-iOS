...
final class WinDelegateTests: XCTestCase {
   ...
   func testOneFail() {
      ...
      // Act
      let result = delegate.vertical(p, on: board) ??
      delegate.horizontal(p, on: board) ??
      delegate.diagonal(p, on: board) ??
      delegate.fourCorners(p, on: board) ??
      delegate.twoByTwo(p, on: board)
